...
describe("pipeline.formatEvent", () => {
  ...
  it("writes a start date-time, taking the given date as local by default and outputting is as UTC by default", () => {
    const event = buildEvent({ start: [2017, 5, 15, 10, 0] });
    const formattedEvent = formatEvent(event);
    const now = dayjs(new Date(2017, 5 - 1, 15, 10, 0))
      .utc()
  ...
  it("writes a start date-time, taking the given date as UTC if requested and outputting is as Local (floating) if requested", () => {
    ...
    const formattedEvent = formatEvent(event);
    const now = dayjs(Date.UTC(2017, 5 - 1, 15, 10, 0)).format(
      "YYYYMMDDTHHmm00"
    );
  ...
  it("writes a html content and folds correctly", () => {
    ...
    const formattedEvent = formatEvent(event);
    expect(formattedEvent).to.contain(
      `X-ALT-DESC;FMTTYPE=text/html:<!DOCTYPE html><html><body><p>This is<br>test<\r\n\tbr>html code.</p></body></html>`
    );
  });
  ...
  it("escapes characters in text types", () => {
    ...
    expect(formattedEvent).to.contain(
      "SUMMARY:colon: semi\\; comma\\, period. slash\\\\"
    );
  });
  ...
  it("writes attendees", () => {
    ...
    expect(formattedEvent).to.contain(
      'ATTENDEE;RSVP=TRUE;CN="Brittany Seaton":mailto:brittany@example.com'
    );
  });
  it("writes a busystatus", () => {
    ...
    expect(formattedEventTent).to.contain(
  ...
  it("writes an organizer", () => {
    ...
    expect(formattedEvent).to.contain(
      foldLine(
        'ORGANIZER;DIR="test-dir-value";SENT-BY="MAILTO:test@example.com";CN="Adam Gibbons":MAILTO:adam@example.com'
      )
    );
  });
  it("writes an alarm", () => {
  ...
  it("writes exception date-time", () => {
    ...
    expect(formattedEvent).to.contain(
      "EXDATE:20000620T010000Z,20000621T010000Z"
    );
  });
